...
const readAndParseSync = (filePath, options = {}) => {
  ...
  if (isJsonPath(filePath)) {
    ...
    return YAML.load(
      contents.toString(),
      mergeRight(options, { filename: filePath }),
    );
        ...
        if (/\${env:(\w*[\w.-_]+)}/g.test(match)) {
          ...
          if (match === value) {
            ...
            newValue = value.replace(
              match,
              process.env[referencedPropertyPath[1]],
            );
          }
        }
      }
...
const isYamlFile = filename => {
  return (
    (filename && filename.endsWith('yaml')) ||
  ...
  if (isYaml) {
    try {
      ...
      if (e.name !== 'YAMLException') {
        ...
        throw new Error(
          `The serverless.yml file has incorrect format. Details: ${e.message}`,
        );
      }
    }
  } else {
...
const getOutputs = allComponentsWithOutputs => {
  ...
  for (const [instanceName, component] of Object.entries(
    allComponentsWithOutputs,
  )) {
...
const setDependencies = allComponents => {
  ...
  for (const component of Object.values(allComponents)) {
    const dependencies = traverse(component.inputs).reduce((accum, value) => {
      ...
      if (matches) {
        for (const match of matches) {
          const splittedVariableString = match
            .substring(2, match.length - 1)
            .split(':');
          ...
          if (
            allComponents[referencedInstanceName] &&
            !accum.includes(referencedInstanceName)
          ) {
...
const executeGraph = async (
  ...
  if (command === 'remove') {
    cli.sessionStatus(
      `[${successCount + 1}/${componentCount}] 正在删除 ${leaves.join(', ')}`,
    );
  } else {
    cli.sessionStatus(
      `[${successCount + 1}/${componentCount}] 正在部署 ${leaves.join(', ')}`,
    );
  }
...
const parseCliInputs = () => {
  ...
  const cliInputs = rawInputs.reduce((accum, rawInput) => {
    ...
    const value =
      dividedValue.length > 2
        ? dividedValue.slice(1).join('=')
        : dividedValue[1];
...
const loadTencentGlobalConfig = (cli, config = {}) => {
  ...
  const globalTencentCredentials = path.join(
    os.homedir(),
