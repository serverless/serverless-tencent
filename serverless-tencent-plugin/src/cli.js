...
module.exports = class CLI {
  ...
  sessionStart(status, options = {}) {
    ...
    setInterval(() => {
      this._.timerSeconds = Math.floor(
        (Date.now() - this._.timerStarted) / 1000,
      );
    }, 1000).unref();
  ...
  sessionStop(reason, messageOrError = 'Closed') {
    ...
    if (reason === 'error') {
      ...
      content += `${figures.pointerSmall} ${
        messageOrError.message || messageOrError
      }`; // In case an error object was passed in
  ...
  logTypeError(typeErrors) {
    ...
    console.log();
    console.log(
      chalk.gray('可以使用 --noValidation 跳过 serverless 应用配置校验'),
    );
    if (errors.length > 0) {
      process.exit();
    }
  ...
  logAdvertisement() {
    ...
    ad =
      ad +
      os.EOL +
      grey('  • And More: https://github.com/serverless/components');
    ad += os.EOL;
    ad =
      ad +
      os.EOL +
  ...
  async _renderEngine() {
    ...
    if (!this._.debug) {
      ...
      content += `${this._.statusColor(
        figures.pointerSmall,
      )} ${this._.statusColor(this._.status)}`;
