...
const checkBasicConfigValidation = async dicPath => {
  ...
  if (!instanceFile.name) {
    throw new Error(
      '在serverless配置文件中没有发现实例名称("name"字段)，请检查。',
    );
  }
  ...
  if (instanceFile.inputs) {
    ...
    if (instanceFile.inputs.src) {
      if (typeof instanceFile.inputs.src === 'string') {
        ...
        instanceFile.inputs.src = path.resolve(
          directoryPath,
          instanceFile.inputs.src,
        );
        ...
        if (instanceFile.inputs.src.src) {
          ...
          instanceFile.inputs.src.src = path.resolve(
            directoryPath,
            instanceFile.inputs.src.src,
          );
        }
        if (instanceFile.inputs.src.dist) {
          instanceFile.inputs.originDist = instanceFile.inputs.src.dist;
          instanceFile.inputs.src.dist = path.resolve(
            directoryPath,
            instanceFile.inputs.src.dist,
          );
        }
      }
    }
...
const getDirForInvokeCommand = async (root, functionAlias) => {
  ...
  const multiScfInstances = instances.filter(
    instance => instance.component === 'multi-scf',
  );
  const scfInstances = instances.filter(
    instance => instance.component === 'scf',
  );
  if (scfInstances.length + multiScfInstances.length === 0) {
    throw new Error(
      '没有找到可执行的函数目录，请使用 --target 指定或检查后再试',
    );
    ...
    throw new Error(
      '目录中存在多个 SCF 组件，请使用 --target 指定目录或检查后再试',
    );
  }
  return instanceDir;
};
...
`;
  ...
  const knownPackages = supportedComponents.filter(value =>
  ...
  if (knownPackages.length === 1) {
    ...
    const result = await inquirer.prompt({
      message:
        '在 package.json 里发现以下依赖，选择您希望创建的 serverless 的应用类型',
      type: 'list',
      name: 'ymlType',
...
const clientUidDefaultPath = path.join(
  os.homedir(),
  '.serverless/tencent/client_uid-credentials',
);
