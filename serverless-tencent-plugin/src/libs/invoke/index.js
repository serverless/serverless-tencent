...
module.exports = async (config, cli, command) => {
  ...
  if (runningTemplate(instanceDir)) {
    try {
      instanceDir = await utils.getDirForInvokeCommand(
        instanceDir,
        functionAlias,
      );
    } catch (e) {
  ...
  try {
    ...
    if (
      !componentType.startsWith('scf') &&
      !componentType.startsWith('multi-scf')
    ) {
      cli.log(
        `Serverless: ${chalk.yellow(
          'Inovke 命令仅能在 scf 或者 multi-scf 组件目录中调用',
        )}`,
      );
    ...
    try {
      ...
      if (res.retMsg) {
        ...
        try {
          ...
          cli.log(
            inspect(retJson, { depth: Infinity, colors: true, compact: 0 }),
          );
        } catch (error) {
          cli.log(retMsg);
        }
