...
const checkRuntime = (requiredRuntime, cli) => {
  ...
  if (requiredRuntime.includes('Nodejs')) {
    ...
    if (
      currentMajorVer !== requiredMajorVer ||
      currentMinorVer !== requiredMinorVer
    ) {
      cli.log(
    ...
    try {
      ...
      throw new Error(
        `检查当前环境的Python 运行时出错，错误信息: ${e.message}`,
      );
    }
    ...
    colorLog(
      '当前命令只支持 Node.js 和 Python 运行时，其他运行时暂不支持。',
      'yellow',
      cli,
    );
  }
...
const summaryOptions = (config, instanceYml, cli) => {
  ...
  if (context && (contextPath || x)) {
    colorLog(
      '不能同时指定 context 与 contextPath，请检查后重试。',
      'yellow',
      cli,
    );
  }
  ...
  if (component.startsWith('scf')) {
    ...
    if (!inputsHandler) {
      colorLog(
        '调用函数未指定，请检查 serverless.yml 中的 hanlder 配置后重试。',
        'yellow',
        cli,
      );
    }
