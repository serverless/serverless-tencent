...
const resolveEvents = (component, inputs) => {
  ...
  if (component === 'scf') {
    ...
    const types = inputs.triggers.reduce(
      (total, curr) => [...total, curr.type],
      [],
    );
    return [types.length, types];
  }
...
module.exports = async ({
  ...
  try {
    ...
    const { value: clientUid } = await writeClientUid(undefined, {
      ciName,
      command,
    }); // get client uid, if it doesn't exist, create one firstly.
    ...
    if (rootConfig) {
      ...
      if (rootConfig.component) {
        ...
        if (rootConfig.component === 'scf') {
          ...
          payload.functions_count = Object.keys(
            rootConfig.inputs.functions,
          ).length;
        }

        const resolveEventsResult = resolveEvents(
    ...
    if (configs) {
      for (const config of configs) {
        ...
        if (config.component === 'scf') {
          ...
          config.component === 'multi-scf' &&
          config.inputs &&
          config.inputs.functions
        ) {
