...
const publish = async (config, cli) => {
  ...
  finalServerlessFile.name =
    finalServerlessFile.name || finalServerlessFile.service;
  ...
  finalServerlessFile.org =
    finalServerlessFile.org || (await utils.getDefaultOrgName());

  // Presentation
  cli.logRegistryLogo();
  cli.log(
    `发布中 "${finalServerlessFile.name}@${
...
const getPackage = async (config, cli) => {
  ...
  cli.log();
  cli.log(
    `${data.type === 'template' ? 'Template' : 'Component'}: ${packageName}`,
  );
...
const listFeatured = async (config, cli) => {
  ...
  if (featuredTemplates.length > 0) {
    ...
    for (const featuredTemplate of featuredTemplates) {
      ...
      if (
        featuredTemplate['description-i18n'] &&
        featuredTemplate['description-i18n']['zh-cn']
      ) {
  ...
  cli.sessionStop(
    'close',
    '查看更多: https://github.com/serverless-components?q=tencent',
  );
  return null;
};
