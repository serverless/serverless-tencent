...
module.exports = async (config, cli, command) => {
  ...
  const closeHandler = async () => {
    ...
    const deployedInstance = await deploy(
      sdk,
      instanceYaml,
      instanceCredentials,
    );
  ...
  const instanceCredentials = await utils.loadInstanceCredentials(
    instanceYaml.stage,
  );
  ...
  try {
    ...
    if (config.filter && config.filter === 'app' && filter.instanceName) {
      ...
      cli.log(
        'Enabling filtering at the activity at the application level',
        'grey',
      );
      cli.log();
    }
    ...
    watcher.on('ready', async () => {
      ...
      if (
        ...
        if (
          ...
          runtimeInfo &&
          region &&
          chinaUtils.doesRuntimeSupportDebug(runtimeInfo)
        ) {
          functionInfoStore = {
            functionName: lambdaArn,
            namespace: namespaceInfo,
